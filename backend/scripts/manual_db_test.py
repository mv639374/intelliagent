...
async def run_db_test():
    ...
    async with AsyncSessionLocal() as session:
        async with session.begin():  # Start a transaction
            ...
            # 1. Create a User
            test_user = User(
                username="testuser", email="test@example.com", hashed_password="fakehashedpassword", role=UserRole.ADMIN
            )
            ...
            test_project = Project(
                name="Test Project", description="A project for manual testing.", owner_id=test_user.id
            )
